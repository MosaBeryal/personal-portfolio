...
const skillCategories = [
  { title: "Frontend", icon: Code, color: "text-blue-400", skills: [ { name: "JavaScript", level: 90 }, { name: "TypeScript", level: 85 }, { name: "React.js", level: 95 }, { name: "Redux", level: 80 } ] },
  { title: "Styling", icon: Palette, color: "text-purple-400", skills: [ { name: "CSS", level: 90 }, { name: "Tailwind CSS", level: 95 }, { name: "Bootstrap", level: 85 }, { name: "Chakra UI", level: 88 } ] },
  { title: "Cloud", icon: Cloud, color: "text-green-400", skills: [ { name: "Azure", level: 75 }, { name: "AWS", level: 70 }, { name: "Firebase", level: 80 } ] },
  { title: "Tools", icon: Wrench, color: "text-amber-400", skills: [ { name: "D3.js", level: 82 }, { name: "Git", level: 90 }, { name: "Next.js", level: 78 }, { name: "Leaflet.js", level: 75 } ] }
];
...
export function SkillsSection() {
  ...
  return (
    <section id="skills" ref={ref} className="relative py-20 overflow-hidden bg-muted/30">
              ...
              <Card className="relative flex flex-col h-full overflow-hidden rounded-3xl border border-white/20 bg-white/10 backdrop-blur-2xl shadow-2xl hover:scale-[1.03] transition-transform duration-300">
                <CardContent className="flex flex-col flex-1 p-6">
                  ...
                  <div className="flex flex-col items-center mb-6 z-10">
                    ...
                    <h3 className="mt-4 text-xl font-semibold text-foreground">{cat.title}</h3>
                  </div>

                  <motion.div className="space-y-4 z-10 flex-1" variants={staggerContainer}>
                    {cat.skills.map((skill, i2) => (
                      <SkillItem key={i2} skill={skill} isVisible={isVisible} delay={i2 * 200} />
                    ))}
                  </motion.div>
                </CardContent>
              </Card>
